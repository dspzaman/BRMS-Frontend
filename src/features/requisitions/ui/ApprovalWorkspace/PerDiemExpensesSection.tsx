...
import { updatePerDiemBudgetAssignment } from '../../api/requisitionApi';
import type {
...
interface PerDiemExpensesSectionProps {
  items: PerDiemExpenseLineItemResponse[];
...
export default function PerDiemExpensesSection({
  items,
  availableBudgetLines,
  onBudgetAssign,
  requisitionId,
  onRefresh,
}: PerDiemExpensesSectionProps) {
  ...
  // Fetch per diem expense types (categories with code 5831)
  const { data: perDiemExpenseTypes, isLoading: isLoadingTypes } = usePerDiemExpenseTypes();
    ...
    setSelectedItems((prev) => {
      ...
      if (prev.length > 0) {
        ...
        if (firstSelectedItem && firstSelectedItem.program !== item.program) {
          toast.warning('Cannot select per diem expenses from different programs. Please select items from the same program only.');
  ...
  const applyBulkAssignment = async () => {
    ...
    // Get the expense code ID for per diem (5831)
    const perDiemExpenseCodeId = perDiemExpenseTypes?.[0]?.expense_code_id;
    if (!perDiemExpenseCodeId) {
      toast.error('Per diem expense code not found');
      return;
    }
    ...
    try {
      ...
      for (const itemId of selectedItems) {
        const item = items.find((i) => i.id === itemId);
        if (item) {
          await updatePerDiemBudgetAssignment(
            requisitionId,
            itemId,
            {
          ...
          onBudgetAssign({
            ...
            expense_code: perDiemExpenseCodeId,
            budget_line_item: Number(bulkBudgetLine),
          });
        }
      }
  ...
  // Filter budget lines based on selections (always filter by per diem code 5831)
  const filteredBudgetLines = selectedProgramId
    ? availableBudgetLines.filter(
        (bl) =>
          ...
          bl.expense_code_id === perDiemExpenseCodeId // Always filter by per diem code
      )
    : [];
    ...
    <div>
      <div className="flex items-center justify-between mb-3">
        <h3 className="text-lg font-semibold text-gray-900">
          Per Diem Expenses ({items.length})
        </h3>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {items.map((item) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-sm text-gray-900">{item.meal_date}</td>
